...
import os
os.environ["TF_CPP_MIN_LOG_LEVEL"] = "2"  # 0 = todos, 1 = INFO, 2 = WARNING, 3 = ERROR
...
def grad_cam(array):
    ...
    # Detectar salida única o múltiple
    output_tensor = model.output[0] if isinstance(model.output, list) else model.output

    # Grad-CAM con GradientTape
    grad_model = tf.keras.models.Model(
        [model.inputs], [model.get_layer("conv10_thisone").output, output_tensor]
    )
    ...
    # Calcular promedio de gradientes por canal
    pooled_grads = tf.reduce_mean(grads, axis=(0, 1, 2))

    # Multiplicar canales por su importancia
...
def predict(array):
    ...
    prediction = np.argmax(model.predict(batch_array_img,verbose=0))
...
class App:
    ...
    def create_pdf(self):
        # Capturar pantalla (solo la parte visible)
        ...
        ID = f"Reporte{self.reportID}.jpg"
        screenshot.save(ID)
